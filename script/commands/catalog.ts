...
export const COMMANDS = {
    ...
    'template:check': command('', 'Check stable private template releases.', '', 'Reads private release metadata; requires HTTPS and TEMPLATE_REPOSITORY_TOKEN via credentials:run.'),
    'template:init': command('<installed-version>', 'Initialize installed template metadata once.', '6.0.2', 'Requires TEMPLATE_REPOSITORY_TOKEN via credentials:run. Pre-6.0.2 applications must use their old updater first.'),
    'template:update': command('[version] | --continue <version> | --abort <version>', 'Update the template or resolve/abort staged conflicts.', '', 'Requires TEMPLATE_REPOSITORY_TOKEN via credentials:run, except offline abort. Downloads private releases, merges, installs, and verifies. Does not commit or deploy.'),
