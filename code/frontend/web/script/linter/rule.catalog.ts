...
const CONCEPTS = {
    ...
    styling: {
        context: 'Shared design tokens live in shared/styles. Pico is imported only by shared/styles/main.css and Tabler Icons use the bundled webfont from that same entry. Presentation CSS is scoped, uses tokens, and follows the repository unit contracts instead of introducing parallel frameworks or breakpoints.',
    },
    vue: {
    ...
    'tooling-failure': {
        context: 'The frontend linter must parse TypeScript, Vue SFCs, and CSS before it can review their architecture. A fatal diagnostic means findings for the affected input or entire run are incomplete.',
    },
...
const RULES = {
    ...
    FRONTEND_PARSE_ERROR: define('A frontend source file could not be parsed', 'tooling-failure', 'Architecture evidence requires syntactically valid TypeScript, Vue, and CSS.', 'Repair the reported syntax error and rerun the linter.'),
    ...
    PICO_CDN_FORBIDDEN: define('Pico must not be loaded from a CDN', 'styling', 'The checked-in dependency and build define the reproducible CSS framework version.', 'Remove the CDN reference and use the package import in shared/styles/main.css.'),
    PICO_GLOBAL_IMPORT_MISSING: define('The global Pico import is missing', 'styling', 'Pico is the sole CSS framework and has one global ownership point.', 'Import @picocss/pico from shared/styles/main.css.'),
    PICO_OWNERSHIP: define('Pico is imported outside shared styles', 'styling', 'Only shared/styles/main.css may introduce the global CSS framework.', 'Remove the import and rely on the global shared style entry.'),
    ...
    TABLER_ICON_CDN_FORBIDDEN: define('Tabler Icons must not be loaded from a CDN', 'styling', 'The bundled pinned webfont is the reproducible icon source.', 'Remove the CDN reference and use the bundled stylesheet from shared/styles/main.css.'),
    TABLER_ICON_GLOBAL_IMPORT_MISSING: define('The bundled Tabler icon import is missing', 'styling', 'The icon webfont has one global ownership point alongside shared styles.', 'Import the bundled Tabler stylesheet from shared/styles/main.css.'),
    TABLER_ICON_OWNERSHIP: define('Tabler Icons are imported outside shared styles', 'styling', 'Presentation code consumes ti classes without owning global font CSS.', 'Remove the local import and rely on shared/styles/main.css.'),
