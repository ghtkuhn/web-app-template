...
const CONCEPTS = {
    ...
    'http-contract': {
        context: 'openapi/openapi.yaml is the public HTTP contract. Every literal /api route must be documented. Tests are selected by concrete failure risk, not by route or status counts. Existing request tests must assert deterministic exact outcomes.',
    },
    'test-contract': {
        context: 'Optional module tests live in direct test/*.test.ts files cataloged by the central runner; no minimum test count is required. Tests may inspect their own module internals, but foreign modules are accessed only through public index.ts entries. Production never imports tests.',
    },
    migration: {
...
const RULES = {
    ...
    OPENAPI_PARSE_ERROR: define('The OpenAPI contract could not be parsed', 'http-contract', 'HTTP contract validation requires a valid OpenAPI document.', 'Repair the reported OpenAPI syntax or schema error.'),
