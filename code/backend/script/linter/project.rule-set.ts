...
const ALLOWED_LAYER_DIRECTORIES = [...LAYERS, TEST_DIRECTORY]
    .sort()
    .join(', ');
    ...
    public evaluateStructure(): LintIssueDraft[] {
        ...
        for (const moduleDirectory of this.scanner.listDirectDirectories(
            ...
            for (const directory of this.scanner.listDirectDirectories(
                ...
                if (!LAYERS.has(layer) && layer !== TEST_DIRECTORY) {
                    issues.push(
                        this.issue(
                            directory,
                    ...
                    layer !== TEST_DIRECTORY &&
                    this.scanner.isEmptyDirectory(directory)
                ) {
                    issues.push(
                        this.issue(
                            directory,
